	...
	\section*{Aufgabe 31}
		\subsection*{a)}
			...
			\begin{enumerate}
				...
				\item Für jedes $a,b \in V$ mit $(a,b)\in E$ enthalte $E' \union = {(a,b),(b,a)}$
				\item Für jeden Knoten $a$ für den es gilt : $\neg\exists (a,b)\in E$ enthalte $E' \union= {(b,a)}$ für alle $b \in V/\{a\}$
			\end{enumerate}
		...
		\subsection*{b)}
			...
			Wir nehmen jedes Mal $\underset{gewählter Knoten}{1}+ \ceil{\frac{(n-1)}{2}}$ viele Knoten heraus. Somit \dots .Wir sind also in $log(n)$ Schritten fertig. 
		\subsection*{c)}
